...
def lambda_handler(event, context):
    ...
    s3.put_object(Bucket='${QR}', Key=filename, Body=img_bytes, ContentType='image/png', ACL='public-read')
    
    # Generate the URL of the uploaded QR code
    location = s3.get_bucket_location(Bucket='${QR}')['LocationConstraint']
    region = '' if location is None else f'{location}'
    qr_code_url = f"https://{'${QR}'}.s3{region}.amazonaws.com/{filename}"
